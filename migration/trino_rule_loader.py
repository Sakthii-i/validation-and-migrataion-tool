...
class TrinoRuleEngine:
    ...
    def _rewrite_cross_join_unnest_static(sql: str) -> str:
        # CROSS JOIN UNNEST(arr) WITH ORDINALITY AS alias(value_col, ord_col)
        #   -> LATERAL VIEW POSEXPLODE(arr) alias AS ord_col, value_col
        # Must run BEFORE the generic (aliasless) WITH ORDINALITY rules below,
        # or those consume the "WITH ORDINALITY" text first and leave this
        # trailing "AS alias(...)" as untouched, invalid Databricks syntax
        # (Databricks' LATERAL VIEW column-alias list is a plain comma list,
        # not a parenthesized "alias(col1, col2)" form).
        # POSEXPLODE always returns (position, value) in that order, while
        # Trino's WITH ORDINALITY names the value column first and the
        # ordinality (position) column second — so the two names must be
        # swapped, not just carried over positionally, or they'd silently
        # end up holding each other's values.
        sql = re.sub(
            r"\bCROSS\s+JOIN\s+UNNEST\s*\(\s*([^)]+?)\s*\)\s+WITH\s+ORDINALITY\s+AS\s+(\w+)\s*\(\s*(\w+)\s*,\s*(\w+)\s*\)",
            r"LATERAL VIEW POSEXPLODE(\1) \2 AS \4, \3",
            sql,
            flags=re.IGNORECASE,
        )
        # UNNEST(arr) WITH ORDINALITY AS alias(value_col, ord_col) — no CROSS JOIN prefix
        sql = re.sub(
            r"\bUNNEST\s*\(\s*([^)]+?)\s*\)\s+WITH\s+ORDINALITY\s+AS\s+(\w+)\s*\(\s*(\w+)\s*,\s*(\w+)\s*\)",
            r"POSEXPLODE(\1) \2 AS \4, \3",
            sql,
            flags=re.IGNORECASE,
        )
        # CROSS JOIN UNNEST(arr) WITH ORDINALITY -> LATERAL VIEW POSEXPLODE(arr)  (no alias)
        sql = re.sub(
    ...
    def _build_pattern_rules(self, rules_list: List[Dict[str, str]]) -> List[Dict]:
        builtins: List[Dict] = [
            ...
            {
                # Trino TRY(expr) suppresses any runtime error in expr, not just
                # casts — there's no single Databricks equivalent, and
                # `TRY_CAST(expr)` (missing `AS type`) is invalid syntax anyway.
                # Flag for manual review instead of emitting a guaranteed parse error.
                'pattern': r'\bTRY\s*\(([^()]+)\)',
                'replacement': r'(\1) /* TODO: Trino TRY(expr) has no single Databricks equivalent; '
                                r'use TRY_CAST/TRY_DIVIDE/etc. for the specific failure mode being guarded against */',
            ...
            # Arg capture allows one level of nested parens (e.g. a function
            # call as an argument) so a comma inside a nested call isn't
            # mistaken for the top-level argument separator.
            {'pattern': r'\bARRAY_POSITION\s*\(\s*((?:[^,()]|\([^()]*\))+?)\s*,\s*((?:[^()]|\([^()]*\))+)\)', 'replacement': r'array_position(\1, \2)', 'flags': re.IGNORECASE, 'priority': 9},
            {'pattern': r'\bCONTAINS\s*\(\s*((?:[^,()]|\([^()]*\))+?)\s*,\s*((?:[^()]|\([^()]*\))+)\)', 'replacement': r'array_contains(\1, \2)', 'flags': re.IGNORECASE, 'priority': 9},
            ...
            # ── JSON FUNCTIONS ───────────────────────────────────────────
            {'pattern': r'\bJSON_EXTRACT_SCALAR\s*\(\s*((?:[^,()]|\([^()]*\))+?)\s*,\s*((?:[^()]|\([^()]*\))+)\)', 'replacement': r'get_json_object(\1, \2)', 'flags': re.IGNORECASE, 'priority': 10},
            {'pattern': r'\bJSON_EXTRACT\s*\(\s*((?:[^,()]|\([^()]*\))+?)\s*,\s*((?:[^()]|\([^()]*\))+)\)', 'replacement': r'get_json_object(\1, \2)', 'flags': re.IGNORECASE, 'priority': 10},
            ...
            # ── STRING FUNCTIONS ─────────────────────────────────────────
            {'pattern': r'\bSTRPOS\s*\(\s*((?:[^,()]|\([^()]*\))+?)\s*,\s*((?:[^()]|\([^()]*\))+)\)', 'replacement': r'LOCATE(\2, \1)', 'flags': re.IGNORECASE, 'priority': 9,
            ...
            {
                # Delimiter arg matches a quoted string literal as a whole
                # unit (so a comma *inside* the delimiter, e.g. SPLIT_PART(s,
                # ',', 2), isn't mistaken for the argument separator) or a
                # bare non-comma expression otherwise.
                'pattern': r"\bSPLIT_PART\s*\(\s*((?:[^,()]|\([^()]*\))+?)\s*,\s*('(?:[^'\\]|\\.)*'|[^,]+?)\s*,\s*([^)]+)\)",
            ...
            {'pattern': r'\bMOD\s*\(\s*((?:[^,()]|\([^()]*\))+?)\s*,\s*((?:[^()]|\([^()]*\))+)\)', 'replacement': r'\1 % \2', 'flags': re.IGNORECASE, 'priority': 8},
            ...
            # format_datetime(ts, fmt) → date_format(ts, fmt) — same arg order in both engines, no swap needed.
            {'pattern': r'\bFORMAT_DATETIME\s*\(', 'replacement': 'date_format(', 'flags': re.IGNORECASE, 'priority': 9},
        ...
        for row in rules_list:
            ...
            # TRY(expr) needs the `AS type` argument-restructuring handled by
            # the dedicated hardcoded rules below, not a blind name rename —
            # skip it here so it can't compile into a naive prefix rule, and
            # (more importantly) so it never enters csv_base_fns and deletes
            # those hardcoded TRY(...) rules via the override filter further down.
            if fn_name.upper() == 'TRY':
                continue
    ...
    def _find_matching_paren(text: str, open_idx: int) -> int:
        ...
        over parens/commas inside single-quoted string literals. -1 if unbalanced."""
        depth = 0
        in_string = False
    ...
    def _split_top_level_commas(text: str) -> List[str]:
        ...
        commas nested inside parens or single-quoted string literals."""
        parts: List[str] = []
        depth = 0
        in_string = False
        start = 0
    ...
    @classmethod
    def _rewrite_calls(cls, sql: str, func_name: str, handler) -> str:
        """
        Replace every call to func_name(...) in sql using balanced-paren-aware
        argument extraction (unlike a plain [^,)]+ regex, this doesn't get
        confused when an argument itself contains parens, e.g. a nested CAST
        or function call). handler(args_list) returns the replacement text,
        or None to leave that particular call untouched.
        """
        ...
        search_from = 0
        while True:
            m = pattern.search(sql, search_from)
            if not m:
                break
            open_idx = m.end() - 1
            close_idx = cls._find_matching_paren(sql, open_idx)
            if close_idx == -1:
                search_from = m.end()
                continue
            args = cls._split_top_level_commas(sql[open_idx + 1:close_idx])
    ...
    @classmethod
    def _rewrite_date_add(cls, sql: str) -> str:
        # Handles both the pre-transpile Trino syntax — DATE_ADD('year', n, date) —
        # and the post-transpile Databricks form sqlglot's direct transpile emits
        # for units with no dedicated Databricks function — DATEADD(YEAR, n, date).
        # DATEADD()/TIMESTAMPADD() always return TIMESTAMP in Databricks regardless
        # of unit, which silently upgrades a DATE column/result to a timestamp, so
        # DAY/WEEK/MONTH/QUARTER/YEAR are routed to DATE_ADD/ADD_MONTHS instead,
        # both of which preserve DATE-ness. HOUR/MINUTE/SECOND correctly stay
        # timestamp-typed, since sub-day arithmetic implies a time component.
        def handle(args: List[str]):
            ...
            if unit == 'DAY':
                return f"DATE_ADD({expr}, {amount})"
            if unit == 'WEEK':
                return f"DATE_ADD({expr}, ({amount}) * 7)"
            if unit == 'MONTH':
                return f"ADD_MONTHS({expr}, {amount})"
            if unit == 'QUARTER':
                return f"ADD_MONTHS({expr}, ({amount}) * 3)"
            if unit == 'YEAR':
                return f"ADD_MONTHS({expr}, ({amount}) * 12)"
            if unit in ('HOUR', 'MINUTE', 'SECOND'):
                dbx_unit = {'HOUR': 'HOURS', 'MINUTE': 'MINUTES', 'SECOND': 'SECONDS'}[unit]
                return f"({expr} + INTERVAL {amount} {dbx_unit})"
            return None

        sql = cls._rewrite_calls(sql, 'DATE_ADD', handle)
        sql = cls._rewrite_calls(sql, 'DATEADD', handle)
        return sql

    @classmethod
    def _rewrite_date_diff(cls, sql: str) -> str:
        # DATE_DIFF('unit', d1, d2) -> Databricks equivalent. MONTH/QUARTER/YEAR use
        # calendar-boundary counting, not MONTHS_BETWEEN (which prorates by
        # day-of-month and gives off-by-one results near month edges, e.g.
        # Jan 31 -> Feb 1 should be 1, not 0).
        def handle(args: List[str]):
            if len(args) != 3:
                return None
            unit = args[0].strip().strip("'\"").upper()
            ...
            if unit == 'WEEK':
                return f"FLOOR(DATEDIFF({d2}, {d1}) / 7)"
            if unit == 'MONTH':
                return f"((YEAR({d2}) - YEAR({d1})) * 12 + (MONTH({d2}) - MONTH({d1})))"
            if unit == 'QUARTER':
                return f"((YEAR({d2}) - YEAR({d1})) * 4 + (QUARTER({d2}) - QUARTER({d1})))"
            if unit == 'YEAR':
                return f"(YEAR({d2}) - YEAR({d1}))"
            if unit in ('HOUR', 'MINUTE', 'SECOND'):
                return f"TIMESTAMPDIFF({unit}, {d1}, {d2})"
            return None

        return cls._rewrite_calls(sql, 'DATE_DIFF', handle)

    @classmethod
    def _rewrite_date_trunc(cls, sql: str) -> str:
        ...
        def handle(args: List[str]):
            if len(args) != 2:
                return None
            unit = args[0].strip().strip("'\"").upper()
            if unit not in ('DAY', 'WEEK', 'MONTH', 'QUARTER', 'YEAR', 'HOUR', 'MINUTE', 'SECOND'):
                return None
            return f"DATE_TRUNC('{unit}', {args[1]})"

        sql = cls._rewrite_calls(sql, 'DATE_TRUNC', handle)
        return sql
